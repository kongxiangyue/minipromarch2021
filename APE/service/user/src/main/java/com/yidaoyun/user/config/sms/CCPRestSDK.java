...
import com.yidaoyun.user.config.sms.utils.CcopHttpClient;
import com.yidaoyun.user.config.sms.utils.DateUtil;
import com.yidaoyun.user.config.sms.utils.EncryptUtil;
import com.yidaoyun.user.config.sms.utils.LoggerUtil;
...
public class CCPRestSDK {
     ...
     * @param welcomePrompt  可选参数 wav格式的文件名，欢迎提示音，在播放验证码语音前播放此内容，配合verifyCode使用，默认值空，当playVerifyCode为空有效。
     * @param playVerifyCode 可选参数 wav格式的文件名，语音验证码的内容全部播放此节点下的全部语音文件，也就是实现了语音验证码功能播放用户自己的语音文件，该参数和verifyCode二者不能同时为空，当二者都不为空时优先使用playVerifyCode。
     ...
     * @param txtSpeed   可选参数 文本转语音后的发音速度，取值范围：-500至500，当mediaTxt有效才生效，默认值为0。
     * @param txtVolume  可选参数 文本转语音后的音量大小，取值范围：-20至20，当mediaTxt有效才生效，默认值为0。
     * @param txtPitch   可选参数 文本转语音后的音调，取值范围：-500至500，当mediaTxt有效才生效，默认值为0。
     * @param txtBgsound 可选参数 文本转语音后的背景音编号，目前云通讯平台支持6种背景音，1到6的六种背景音编码，0为不需要背景音。暂时不支持第三方自定义背景音。当mediaTxt有效才生效。
