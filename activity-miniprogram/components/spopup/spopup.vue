<template>
  <view :class="['s-popup',positionClass,visibleClass,effectClass,customClass]" :style="styleZindex+styleDuration">
    <template v-if="mask">
      <view class="s-popup-mask" @touchmove.stop.prevent="e=>e.preventDefault()" @click="maskClose && hide()" :style="'background-color: rgba(0, 0, 0, '+maskOpacity+');'"></view>
    </template>
    <view v-if="preventTouchmove" class="s-popup-wrapper" @touchmove.stop.prevent="e=>e.preventDefault()">
      <slot></slot>
    </view>
